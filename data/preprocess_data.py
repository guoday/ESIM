...
def build_sequence(filepath, dst_dir, isTest=False):
    ...
    with open(filepath) as f, \
         ...
         open(os.path.join(dst_dir, 'pairID_%s'%filename), 'w') as f4:
...
def build_dep_dictionary(filepaths, dst_path, lowercase=False):
    word_freqs = OrderedDict()
    for filepath in filepaths:
        print 'Processing', filepath
        with open(filepath, 'r') as f:
    ...
    worddict = OrderedDict()
    worddict['_PAD_'] = 0 # default, padding 
    worddict['_SEF_'] = 1 # out-of-vocabulary
...
if __name__ == '__main__':
    ...
    multinli_dir = os.path.join(base_dir, 'multinli/multinli_0.9')
    make_dirs([dst_dir])

    #build_sequence(os.path.join(multinli_dir, 'multinli_0.9_train.txt'), dst_dir)
    #build_sequence(os.path.join(multinli_dir, 'multinli_0.9_dev_matched.txt'), dst_dir)
    ...
    #build_dictionary([os.path.join(dst_dir, 'premise_multinli_0.9_train.txt'), 
    #                  os.path.join(dst_dir, 'hypothesis_multinli_0.9_train.txt')], 
    #                  os.path.join(dst_dir, 'vocab_cased.pkl'))
    build_dep_dictionary([ '../denpency_data/premise_multinli_0.9_train_dep.txt', 
                      '../denpency_data/hypothesis_multinli_0.9_train_dep.txt'], 
                      '../denpency_data/dep_vocab_cased.pkl')